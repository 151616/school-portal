...
const runDiagnosticsHelper = async (auth, db, addToast) => {
  ...
  try {
    ...
    // Test write to diagnostics node (non-destructive; will remove after)
    try {
      const diagRef = push(ref(db, 'diagnostics/testWrite'));
      await set(diagRef, { ts: Date.now(), by: auth.currentUser.uid });
      console.log('Write to diagnostics/testWrite succeeded. Cleaning up...');
      await set(diagRef, null);
      console.log('Diagnostics write cleanup successful.');
    } catch (err) {
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <button className="btn btn-ghost" onClick={(e) => { const b = e.currentTarget; b.classList.add('pulse'); setTimeout(() => b.classList.remove('pulse'), 260); runDiagnosticsHelper(auth, db, addToast); }} style={{ marginLeft: 10 }}><AlertIcon className="icon"/> Run Diagnostics</button>
          </div>
        </div>
